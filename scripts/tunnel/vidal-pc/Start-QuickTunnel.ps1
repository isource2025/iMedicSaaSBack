...
function Stop-LegacyFileServers {
	...
	Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
		Where-Object {
			$_.Name -match '^(powershell|pwsh)(\.exe)?$' -and $_.CommandLine -and (
				$_.CommandLine -like '*file-server-runtime*' -or
				$_.CommandLine -like '*file-server.ps1*' -or
				$_.CommandLine -like '*\iMedic\file-server*'
			)
		} |
		ForEach-Object {
	...
	foreach ($procId in (Get-PidsOnPort $Port)) {
		...
		try {
			$p = Get-Process -Id $procId -ErrorAction SilentlyContinue
			if ($p) {
				Write-Host "Liberando puerto $Port (PID $($p.Id) $($p.ProcessName))..."
				Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue
			}
		} catch {}
	}
	...
	try {
		Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue |
			...
			ForEach-Object {
				Write-Host "Kill listener OwningProcess $($_.OwningProcess)..."
	...
	# Si sigue respondiendo el viejo (ps1-unc-*), avisar claro.
	$h = Get-HealthJson "http://127.0.0.1:$Port/health"
	if ($h -and $h -match 'ps1-unc') {
		Write-Host "AVISO: el puerto $Port sigue respondiendo el file server viejo." -ForegroundColor Yellow
		Write-Host "Abrí el Administrador de tareas y mata el PowerShell de C:\ProgramData\iMedic\file-server.ps1" -ForegroundColor Yellow
	}
}
...
function Normalize-Path([string]$p) {
  ...
  # UNC Clarion legacy \\192.168…\Imagenes\Vidal|Vida\… → relativo bajo RootDir
  if ($x -match '(?i)^\\\\[^\\]+\\Imagenes\\[^\\]+\\(.+)$') {
    $x = Join-Path $RootDir $Matches[1]
  } elseif ($x -match '(?i)^\\\\[^\\]+\\Imagenes\\[^\\]+$') {
    $x = $RootDir
  }
  # Ruta relativa (requisitos): PERSONALES\… o {DNI NOMBRE}\…
  $esAbsoluta = ($x -match '^[A-Za-z]:\\') -or ($x -like '\\*')
  if (-not $esAbsoluta) {
    $x = Join-Path $RootDir $x.TrimStart('\')
  }
...
function Find-ExistingFile([string]$p) {
  ...
  # Relativa o UNC legacy → bajo RootDir de ESTA clinica
  $raw = [string]$p
  if ($raw -and ($raw -notmatch '^[A-Za-z]:\\') -and ($raw -notlike '\\*')) {
    $joined = Join-Path $RootDir $raw.TrimStart('\')
    if (-not $names.Contains($joined)) { [void]$names.Add($joined) }
  }
  if ($raw -match '(?i)^\\\\[^\\]+\\Imagenes\\[^\\]+\\(.+)$') {
    $joined = Join-Path $RootDir $Matches[1]
    if (-not $names.Contains($joined)) { [void]$names.Add($joined) }
  }
    ...
    try {
      ...
      if ($req.HttpMethod -eq "POST" -and $route -eq "/upload") {
        ...
        foreach ($part in $parts) {
          ...
          if ($name -eq "path" -and $txt) { $destPath = $txt }
        ...
        if (-not $destPath) {
          ...
          # Relativa / UNC Clarion → Normalize-Path antepone RootDir de ESTA clinica
          $destPath = Normalize-Path (Repair-Utf8Mojibake $destPath)
        }
